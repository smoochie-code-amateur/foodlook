...
function renderDashboard() {
  ...
  html += '<div class="progress-wrap">';
  html += '<div class="progress-header"><span class="progress-label"><span class="row-label">🥩 Білки<span class="macro-tip">М\'ясо, риба, яйця, молочні, бобові, горіхи</span></span></span><span class="progress-value">' + Math.round(tProt) + ' / ' + needs.protTarget + ' г</span></div>';
  html += '<div class="progress-bar"><div class="progress-fill ' + pctColor(protPct) + '" style="width:' + protPct + '%"></div></div>';
  html += '</div>';

  html += '<div class="progress-wrap">';
  html += '<div class="progress-header"><span class="progress-label"><span class="row-label">🧈 Жири<span class="macro-tip">Оливкова олія, горіхи, авокадо, жирна риба, масло</span></span></span><span class="progress-value">' + Math.round(tFat) + ' / ' + needs.fatTarget + ' г</span></div>';
  html += '<div class="progress-bar"><div class="progress-fill ' + pctColor(fatPct) + '" style="width:' + fatPct + '%"></div></div>';
  html += '</div>';

  html += '<div class="progress-wrap">';
  html += '<div class="progress-header"><span class="progress-label"><span class="row-label">🍞 Вуглеводи<span class="macro-tip">Крупи, хліб, овочі, фрукти, макарони, картопля</span></span></span><span class="progress-value">' + Math.round(tCarbs) + ' / ' + needs.carbsTarget + ' г</span></div>';
...
function initEventListeners() {
  ...
  if ('ontouchstart' in window) {
    document.addEventListener('click', function(e) {
      var tip = e.target.closest('.row-label');
      if (tip && tip.querySelector('.macro-tip')) {
        var wasOpen = tip.classList.contains('tip-open');
        document.querySelectorAll('.row-label.tip-open').forEach(function(r) { r.classList.remove('tip-open'); });
